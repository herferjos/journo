...
def registrar_uso():
    ...
    if st.session_state.database.isna().all().all():
        st.session_state.database_tracking = st.session_state.sheet.update(worksheet=f'{st.session_state.email}_tracking', data = pd.DataFrame({'Registro Uso': [st.session_state.time_session],'Transcripción': [st.session_state.transcription2], 'Transcripción editada': [st.session_state.transcripcion_editada], 'Cargo': [st.session_state.X], 'Nombre': [st.session_state.Y], 'Donde': [st.session_state.A], 'Cuando': [st.session_state.B], 'Extra': [st.session_state.Z], 'Anotaciones': [st.session_state.anotaciones_finales], 'Noticia': [st.session_state.noticia_generada], 'Noticia editada': [st.session_state.noticia_editada]}))
    else:                                          
...
def cargar_y_transcribir_audio(audio):
    ...
    st.session_state.transcription1, st.session_state.timestamps = transcribe_audio_splitter(st.session_state.mp3_audio_path)
